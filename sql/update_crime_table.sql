DROP TABLE IF EXISTS dup_chicago_crime;

CREATE TABLE IF NOT EXISTS dup_chicago_crime(
  id BIGINT,
  line_num INT,
  dup_ver INT,
  PRIMARY KEY(dup_ver)
...
DROP TABLE IF EXISTS new_chicago_crime;

CREATE TABLE IF NOT EXISTS new_chicago_crime(
  id BIGINT,
  line_num INT,
  dup_ver INT,
  PRIMARY KEY(dup_ver)
...
--
DROP TABLE IF EXISTS DEDUP_chicago_crime;

CREATE TABLE DEDUP_chicago_crime(
dup_chicago_crime_row_id INTEGER PRIMARY KEY
);

INSERT INTO DEDUP_chicago_crime
SELECT MAX(dup_chicago_crime_row_id)
...
Location
FROM DUP_chicago_crime
JOIN DEDUP_chicago_crime USING (dup_chicago_crimes_all_row_id);
...
LEFT OUTER JOIN DAT_chicago_crime USING (ID)
WHERE chicago_crime_row_id IS NULL;
...
Location
FROM SRC_chicago_crime
JOIN NEW_chicago_crime USING (ID);
...
SELECT
  ...
  NULL AS obs_ts,
  'chicago_crime' AS dataset_name,
  chicago_crime_row_id AS dataset_row_id
FROM
  DAT_chicago_crime
JOIN
  NEW_chicago_crime USING (ID);


--
-- find all changes to existing records
--
DROP TABLE IF EXISTS CHG_chicago_crime;
...
SELECT ID
FROM SRC_chicago_crime S
JOIN DAT_chicago_crime D
USING (ID)
...
--
UPDATE DAT_chicago_crime D
SET end_date = '2013-10-30',
current_flag = FALSE
FROM CHG_chicago_crime C
WHERE D.ID = C.ID
...
FROM
  DAT_chicago_crime D
WHERE 
  M.dataset_row_id = D.chicago_crime_row_id 
...
Location
FROM SRC_chicago_crime
JOIN CHG_chicago_crime USING (ID);

DROP TABLE IF EXISTS DUP_chicago_crime;

CREATE TABLE IF NOT EXISTS DUP_chicago_crime(
dup_chicago_crime_row_id SERIAL,
ID INTEGER,
Case_Number VARCHAR(10),
Orig_Date TIMESTAMP,
Block VARCHAR(50),
...
Longitude FLOAT8,
Location POINT,
PRIMARY KEY(dup_chicago_crime_row_id));

COPY DUP_chicago_crime
(ID,
Case_Number,
Orig_Date,
Block,
