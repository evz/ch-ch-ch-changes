...
class ETL(object):
    ...
    def run(self):
        ...
        filename = self.download_file('chicago-crime', 
                                      'ijzp-q8t2')
        ...
        try:
            ...
            contents.close()
            print(e)
            self.update_meta_table(filename, 'failed - bad data')
            proceed = False
    ...
    def download_file(self, download_type, fourbyfour):
        ...
        if not os.path.exists(filepath):
            ...
            with open(filepath, 'wb') as f:
                ...
                r = requests.get(f'https://data.cityofchicago.org/api/views/{fourbyfour}/rows.csv', params=params, stream=True)
                for chunk in r.iter_content(chunk_size=1024):
                    if chunk:
                        f.write(chunk)
                        f.flush()
            download_duration = str(time.time() - start)
            print(f'{download_type} download took {download_duration} \n')
        return filename
    ...
    def update_iucr_table(self):
        ...
        '''
        filename = self.download_file('iucr', 
                                      'qimd-vs49')
    ...
    def flag_deletions(self):
        ...
        with db.engine.begin() as curs:
            curs.execute(text(update), 
                         {'deleted_on': self.file_date.strftime('%Y-%m-%d')})
